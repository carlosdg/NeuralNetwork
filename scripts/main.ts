class Neuron
{
    ...
    // Is convenient to store the deltas in the neurons
    ...
    constructor( numberOfInputs: number,
                 private isConstantOutputNeuron_: boolean,
                 private outputVal_: number = 1
    ){
    ...
    public setOutput( newVal: number ): void{
    ...
    public feed( inputFeatures: Neuron[] ): Neuron{
    ...
    public calculateOutputLayerDelta ( yReal: number ): Neuron{
    ...
    public calculateHiddenLayerDelta ( nextLayerNeurons: Neuron[], q: number, iAmLastHiddenLayer: boolean = false ): Neuron{
        let sum = 0,
            nextLayerSize = nextLayerNeurons.length;
    ...
    public updateWeights ( learningRate: number, prevLayerNeurons: Neuron[] ): Neuron{
        ...
        for (let i = 0, length = this.weights_.length; i < length; ++i){
...
interface NeuralNetworkTopology{
    ...
    numHiddenLayerNeurons: number[];
    numOutputLayerNeurons: number;
}


class NeuralNetwork
{
    private inputLayer:   Neuron[]   = [];
    private hiddenLayers: Neuron[][] = [];
    private outputLayer:  Neuron[]   = [];

    constructor( topology: NeuralNetworkTopology,
                 private learningRate: number    = 0.01,
                 private numIterationsGD: number = 1000){
        // Input layer
        for (let i = topology.numInputNeurons; i > 0; --i){
        ...
        let numNeuronsPrevLayer = this.inputLayer.length;
        // Create the hidden layers
        for (let i = 0, length = topology.numHiddenLayerNeurons.length; i < length; ++i){
            this.hiddenLayers.push( [] );
            // Construct the neurons in each hidden layer
            for (let j = topology.numHiddenLayerNeurons[i]; j > 0; --j){
        ...
        // Output layer
        for (let i = topology.numOutputLayerNeurons; i > 0; --i){
    ...
    public predict( inputFeatures: number[] ): number[]{
        ...
        let numInputNeurons = this.inputLayer.length;
        ...
        for (let i = 0; i < numInputNeurons - 1; ++i){
        ...
        for (let i = 0, numNeurons = this.hiddenLayers[0].length - 1; i < numNeurons; ++i){
            this.hiddenLayers[0][i].feed( this.inputLayer );
        }
        // Rest of hidden layers
        for (let i = 1, numHiddenLayers = this.hiddenLayers.length; i < numHiddenLayers; ++i){
            for (let j = 0, numNeurons = this.hiddenLayers[i].length - 1; j < numNeurons; ++j){
                this.hiddenLayers[i][j].feed( this.hiddenLayers[i-1] );
            }
        }

        // Feed foward. Output neurons
        let result: number[] = [];  // Variable with the results to return
        for (let i = 0, numNeurons = this.outputLayer.length; i < numNeurons; ++i){
    ...
    public train( X: number[][], Y: number[][] ): NeuralNetwork{
        ...
        for (let gdIteration = this.numIterationsGD; gdIteration > 0; --gdIteration){
            // For each input
            for (let i = 0, numInputs = X.length; i < numInputs; ++i){
                // Make a prediction (output neurons store it and use it in the delta calculation)
                this.predict( X[ i ] );

                // Calculate output layer deltas
                for (let j = 0, numOutputLayerNeurons = this.outputLayer.length; j < numOutputLayerNeurons; ++j){
                ...
                for (let j = 0,
                    lastHiddenLayer = this.hiddenLayers[ this.hiddenLayers.length - 1 ],
                    numNeurons = lastHiddenLayer.length;
                    j < numNeurons;
                    ++j
                ){
                ...
                for (let j = this.hiddenLayers.length - 2; j >= 0; --j){
                    for (let n = 0, numNeurons = this.hiddenLayers[ j ].length; n < numNeurons; ++n){
                ...
                // First hidden layer
                for (let j = 0, numNeurons = this.hiddenLayers[0].length - 1; j < numNeurons; ++j){
                    this.hiddenLayers[0][j].updateWeights( this.learningRate, this.inputLayer );
                }
                // Rest of hidden layers
                for (let j = 1, numHiddenLayers = this.hiddenLayers.length; j < numHiddenLayers; ++j){
                    for (let n = 0, numNeurons = this.hiddenLayers[j].length - 1; n < numNeurons; ++n){
                        this.hiddenLayers[j][n].updateWeights( this.learningRate, this.hiddenLayers[j-1] );
                    }
                }

                // Update output layer weights
                for (let n = 0, numNeurons = this.outputLayer.length; n < numNeurons; ++n){
                    let lastHiddenLayer = this.hiddenLayers[ this.hiddenLayers.length - 1 ];
...
for (let i = 0, numData = irisDataSet.length; i < numData; i+=2){
